...
w <- 'https://en.wikipedia.org/wiki/Category:Lists_of_country_codes' %>%
    ...
    dmap(~gsub('\\n', ', ', .x)) %>%
    setNames(c('tld', 'calling', 'mcc', 'gec', 'gs1.gtin', 'icao.aircraft', 'icao.airport', 'ioc', 'iso2c', 'iso3c', 'iso3n', 'itu.callsign', 'itu.letter', 'itu.maritime', 'license.plate', 'marc', 'stanag', 'nato2c', 'undp', 'wmo'))
...
okfn_c <- okfn %>% select(en_iso = 2, fr_iso = 3, iso2c = 4, iso3c = 5, iso3n = 6,
                          7:10, calling = Dial, 12, gec = FIPS, 14:15, iso4217_3c = 16,
                          iso4217_name = 19, iso4217_3n = 20, 21:24, 27) %>%
    janitor::clean_names()
w_c <- w %>% select(tld, 2:4, 6:20) %>%
    mutate(license.plate = gsub(' \\(.*\\)', '', license.plate)) %>%
    janitor::clean_names()
