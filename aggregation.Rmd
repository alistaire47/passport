...
countries <- full_join(unicode, us_ok_w, by = c('iso3n', 'iso2c', 'alt')) %>% 
    mutate(iso3c = coalesce(iso3c.x, iso3c.y), 
           gec = coalesce(gec.x, gec.y)) %>% 
    select(-matches('\\.'))
...
codes <- data_frame(column = countries_colnames, 
    ...
    mutate(expansion = map(code, safely(NLP::parse_IETF_language_tag), expand = TRUE), 
           ...
           expansion = at_depth(expansion, 2, 
                                ~suppressWarnings(na_if(toString(na.omit(.x)), ''))), 
           expansion = map(expansion, as_data_frame), 
    ...
    mutate(Language = case_when(column == 'alt' ~ NA_character_,
           ...
           name = case_when(
               ...
               column == 'en_iso' ~ 'Official ISO (UN) English name',
               column == 'fr_iso' ~ 'Official ISO (UN) French name',
